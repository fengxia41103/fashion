...
class MyItemEdit (UpdateView):
    ...
    fields = ['name', 'description', 'help_text', 'season',
              'brand', 'color', 'price', 'size_chart', 'is_active']
...
class MyItemListFilter (FilterSet):
    ...
    season = ModelChoiceFilter(
        queryset=MySeason.objects.all().order_by('name'))
    ...
    def get_context_data(self, **kwargs):
        ...
        for f, val in searches.data.iteritems():
            if val and f != "csrfmiddlewaretoken" and f != "page":
                if f == 'brand':
                    context['filters'][
                        'brand'] = MyCRM.objects.get(id=int(val))
                if f == 'season':
                    context['filters'][
                        'season'] = MySeason.objects.get(id=int(val))
                if 'name' in f:
                    context['filters']['name__icontains'] = val

        # vendors included in queryset
        context['vendors'] = [MyCRM.objects.get(id=v) for v in set(
            self.object_list.values_list('brand', flat=True))]
        return context
...
class MyItemDetail(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['same_styles'] = MyItem.objects.filter(
            name=self.object.name, brand=self.object.brand)

        # List all open SO that user can add this item to
        context['sales_orders'] = filter(
            lambda x: x.is_editable, MySalesOrder.objects.all().order_by('id'))

        # related sales orders
        item_invs = MyItemInventory.objects.filter(item=self.object)
        context['related_sales_orders'] = MySalesOrderLineItem.objects.filter(
            item__in=item_invs)

        # related purchase orders
        item_invs = MyItemInventory.objects.filter(item=self.object)
        context['related_purchase_orders'] = MyPurchaseOrderLineItem.objects.filter(
            inv_item__in=item_invs)

        # vendor item form
        context['vendor_items'] = MyVendorItem.objects.filter(
            product=self.object)
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            for member in my_zip.namelist():
                ...
                if len(tmp) < 2:
                    errors.append(
                        (member, 'Format error. Should be "style&color[&index].jpg[|png]."'))
                    continue
                ...
                items = MyItem.objects.filter(
                    name__iexact=style, color__iexact=color, brand=vendor, season=season)
                if not len(items):
                ...
                tmp_file = NamedTemporaryFile(
                    prefix=prefix, suffix=ext, delete=True)
                tmp_file.write(data)
...
class MyVendorDetail(DetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super(DetailView, self).get_context_data(**kwargs)
        seasons = MyItem.objects.filter(
            brand=self.object).values_list('season', flat=True)
        ...
        for season in MySeason.objects.filter(id__in=seasons).order_by('name'):
            ...
            samples = reduce(
                lambda x, y: x + y, [list(item.attachments.all()) for item in items])
            tmp[season] = samples
        context['seasons'] = tmp
        return context
...
def add_to_inventory(storage, quick_notion, out, reason, created_by):
    ...
    # track how many items we have ever shipped to them, so they are not
    # drawable.
    if storage.location.crm.crm_type == 'C':
        withdrawable = False
    else:
        withdrawable = True
...
class MyItemInventoryPhysicalAdd(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['vendor'] = vendor = MyCRM.objects.get(
            id=int(kwargs['vendor']))
        context['season'] = season = MySeason.objects.get(
            id=int(kwargs['season']))

        context['storage'] = storage
        context['storages'] = MyStorage.objects.exclude(id=storage.id)
        context['inv_items'] = MyItemInventory.objects.filter(
            storage=storage, item__brand=vendor, item__season=season).order_by('item__name')
        return context
    ...
    def post(self, request, storage, vendor):
        ...
        for line_id, val in self.request.POST.iteritems():
            if 'inv-item' in line_id:
                inv_item = MyItemInventory.objects.get(
                ...
                items[inv_item.id] = {
                    'item': inv_item, 'qty': int(val), 'on': False}
        for line_id, val in self.request.POST.iteritems():
...
class MySeasonDetail(DetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super(DetailView, self).get_context_data(**kwargs)
        vendors = set(
            MyItem.objects.filter(season=self.object).values_list('brand', flat=True))

        # Get vendor stats
        vendor_stats = []
        for v in [MyCRM.objects.get(id=int(v)) for v in vendors]:
            num_of_items = MyItem.objects.filter(
                season=self.object, brand=v).count()
...
def add_item_to_sales_order(quick_notion, so):
    ...
    for line_no, line in enumerate(quick_notion.split('\n')):
        ...
        for (size, qty) in pat.findall(','.join(tmp[1:])):
            ...
            # Create SO line item
            if so.is_sold_at_cost:
                price = item.converted_cost
            else:
                price = item.price
...
class MySalesOrderAdd(FormView):
    ...
    def get_success_url(self):
        if self.order:
            return reverse_lazy('so_detail', kwargs={'pk': self.order.id})
        else:
            return reverse_lazy('so_list')
    ...
    def form_valid(self, form):
        ...
        if form.cleaned_data['discount']:
            so.discount = form.cleaned_data['discount']
        else:
    ...
    def get_context_data(self, **kwargs):
        ...
        for f, val in searches.data.iteritems():
            if val and f != "csrfmiddlewaretoken" and f != "page":
                if f == 'customer':
                    context['filters'][
                        'customer'] = MyCRM.objects.get(id=int(val))
                if f == 'sales':
                    context['filters']['sales'] = User.objects.get(id=int(val))
        return context
    ...
    def get_context_data(self, **kwargs):
        ...
        for i in MySalesOrderLineItem.objects.filter(order=self.object):
            ...
            if brand not in items:
                items[brand] = {'total_qty': 0, 'total_value': 0, 'items': {}}
            items[brand]['total_qty'] += i.qty
            items[brand]['total_value'] += i.discount_value

            # Get item
            if item not in items[brand]['items']:
                items[brand]['items'][item] = {
                    'so_line_items': [], 'qty': 0, 'value': 0}
        ...
        # related POs
        context['purchase_orders'] = MyPurchaseOrder.objects.filter(
            so=self.object)
...
class MySalesOrderAddItem(TemplateView):
    ...
    def post(self, request):
        ...
        if so.is_sold_at_cost:
            price = item_inv.item.converted_cost
        else:
            price = item_inv.item.price
...
class MySalesOrderToPurchaseOrder(TemplateView):
    ...
    def post(self, request, pk):
        ...
        for i in MySalesOrderLineItem.objects.filter(order=so):
            item = i.item.item
            if item.brand not in items_by_brand:
                items_by_brand[item.brand] = []
    ...
    def get_context_data(self, **kwargs):
        ...
        for f, val in searches.data.iteritems():
            if val and f != "csrfmiddlewaretoken" and f != "page":
                if f == 'so':
                    context['filters'][
                        'so'] = MySalesOrder.objects.get(id=int(val))
        return context
...
class MySalesOrderPaymentAdd(FormView):
    ...
    def get_success_url(self):
        if self.payment:
            return reverse_lazy('so_detail', kwargs={'pk': self.payment.so.id})
        else:
            return reverse_lazy('so_payment_list')
    ...
    def post(self, request, pk):
        ...
        for line_id, qty in self.request.POST.iteritems():
            if 'line-item' in line_id and int(qty):
                line_item = MySalesOrderLineItem.objects.get(
                    id=int(line_id.split('-')[-1]))
...
class MySalesOrderReturnEdit(UpdateView):
    ...
    def post(self, request, pk):
        ...
        for line_id, val in self.request.POST.iteritems():
            if 'line-item' in line_id and int(val):
                line_item = MySalesOrderReturnLineItem.objects.get(
                    id=int(line_id.split('-')[-1]))
    ...
    def get_context_data(self, **kwargs):
        ...
        for f, val in searches.data.iteritems():
            if val and f != "csrfmiddlewaretoken" and f != "page":
                if f == 'vendor':
                    context['filters'][
                        'vendor'] = MyCRM.objects.get(id=int(val))
                if f == 'currency':
                    context['filters'][
                        'currency'] = MyCurrency.objects.get(id=int(val))
                if f == 'product':
                    context['filters'][
                        'item'] = MyItem.objects.get(id=int(val))
...
class MyPurchaseOrderAdd(FormView):
    ...
    def get_success_url(self):
        if self.order:
            return reverse_lazy('po_detail', kwargs={'pk': self.order.id})
        else:
            return reverse_lazy('po_list')
...
class MyInvoiceDetail(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Invoice edit view
        context['invoice_edit_form'] = VendorInvoiceAddForm(
    ...
    def post(self, request, pk):
        ...
        for line_id, val in self.request.POST.iteritems():
            if 'invoice-item' in line_id and int(val):
                line_item = MyInvoiceItem.objects.get(
                    id=int(line_id.split('-')[-1]))
...
class MyInvoiceList (FilterView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for f, val in searches.data.iteritems():
            if val and f != "csrfmiddlewaretoken" and f != "page":
                if f == 'crm':
                    context['filters']['crm'] = MyCRM.objects.get(id=int(val))
        return context
...
class MyInvoiceReceiveAdd(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['items'] = filter(
            lambda x: x.qty_balance > 0, MyInvoiceItem.objects.filter(invoice=self.object))
    ...
    def post(self, request, pk):
        ...
        for line_id, val in self.request.POST.iteritems():
            if 'line-item' in line_id and int(val):
                invoice_item = MyInvoiceItem.objects.get(
                    id=int(line_id.split('-')[-1]))
...
class MyInvoiceReceiveDetail(DetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super(DetailView, self).get_context_data(**kwargs)
        context['items'] = MyInvoiceReceiveItem.objects.filter(
            invoice_receive=self.object).order_by('item__inv_item__id')
        return context
    ...
    def get_context_data(self, **kwargs):
        ...
        for f, val in searches.data.iteritems():
            if val and f != "csrfmiddlewaretoken" and f != "page":
                if f == 'invoice':
                    context['filters'][
                        'invoice'] = MyInvoice.objects.get(id=int(val))
        return context
...
class MyVendorSampleInvoiceAdd(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['formset'] = formset_factory(
            VendorSampleInvoiceLineItemAddForm, extra=self.extra)
    ...
    def get_context_data(self, **kwargs):
        ...
        for f, val in searches.data.iteritems():
            if val and f != "csrfmiddlewaretoken" and f != "page":
                if f == 'po':
                    context['filters'][
                        'po'] = MyPurchaseOrder.objects.get(id=int(val))
        return context
...
class ReportCustomerAR(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        context = super(TemplateView, self).get_context_data(**kwargs)
        data = sorted(MyCRM.objects.customers().order_by(
            'name'), lambda x, y: x.account_receivable > y.account_receivable)
        context['data'] = data
...
class ReportVendorAP(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        context = super(TemplateView, self).get_context_data(**kwargs)
        data = sorted(MyCRM.objects.vendors().order_by(
            'name'), lambda x, y: x.account_payable > y.account_payable)
        context['data'] = data
...
class ReportTopProductBySO(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        context = super(TemplateView, self).get_context_data(**kwargs)
        context['data'] = data = MyItemInventory.objects.rank_by_so_qty(
            top=int(kwargs['top']))
        context['limited_to'] = kwargs['top']

        # compute alternative charts that can be available based on what's
        # current
        alternatives = ['10', '25', '50']
        try:
            alternatives.remove(kwargs['top'])
        except:
            pass
...
class ReportTopProductByPO(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        context = super(TemplateView, self).get_context_data(**kwargs)
        context['data'] = data = MyItemInventory.objects.rank_by_po_qty(
            top=int(kwargs['top']))
        context['limited_to'] = kwargs['top']
...
class ReportTopSOByQtyBalance(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        context = super(TemplateView, self).get_context_data(**kwargs)
        context['data'] = data = MySalesOrder.objects.rank_by_qty_balance(
            top=int(kwargs['top']), reverse=True)
        context['limited_to'] = kwargs['top']
...
class ReportPhysicalInventoryList (FilterView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for audit in self.object_list.order_by('-id'):
            if audit.inv not in latest:
                latest[audit.inv] = audit
        ...
        for f, val in searches.data.iteritems():
            if val and f != "csrfmiddlewaretoken" and f != "page":
                if f == 'inv__item__brand':
                    context['filters'][
                        'inv__item__brand'] = MyCRM.objects.get(id=int(val))
        return context
...
class MyShoppingCartUpdate(TemplateView):
    ...
    def post(self, request):
        cart, created = MyShoppingCart.objects.get_or_create(
            user=request.user, is_open=True)
        ...
        for line_id, qty in self.request.POST.iteritems():
            if 'inv-item' in line_id and qty:
                ...
                inv_item = MyItemInventory.objects.get(
                    id=int(line_id.split('-')[-1]))
                shopping_cart_item, created = MyShoppingCartItem.objects.get_or_create(
                    cart=cart, inv_item=inv_item)
                if qty:
